...
import { initCameraForWebview, handleOrientationChange, requestCameraPermission } from "@/utils/webview-camera-helper";
export default function TeacherAttendanceScan() {
 ...
 const [cameraOrientation, setCameraOrientation] = useState("user"); // Default to front camera
 ...
 const getPlatformInfo = () => {
   ...
   // Expanded WebView detection
   const isWebView = () => {
     if (typeof window === 'undefined') return false;
     const userAgent = window.navigator.userAgent.toLowerCase();

     // Standard WebView detection patterns
     ...
     // Check for specific WebView bridge objects
     const hasWebViewBridge = typeof window.ReactNativeWebView !== 'undefined' ||
                             typeof window.webkit?.messageHandlers !== 'undefined';

     // Additional Android WebView indicators
     const hasAndroidWebViewIndicators = /; wv\)/.test(userAgent) ||
                                        /\sVersion\//.test(userAgent);

     return isAndroidWebView || isIOSWebView || hasWebViewBridge || hasAndroidWebViewIndicators;
   };
 ...
 useEffect(() => {
   ...
   // Request camera permission early to prepare for camera access
 ...
 const startCamera = async () => {
   try {
     ...
     // Use the enhanced camera initialization function for WebView
     const stream = await initCameraForWebview({
       width: 640,
       height: 480,
     ...
     // Setup camera orientation change handler for mobile devices
     const cleanup = handleOrientationChange(videoRef.current);
     // Get location
     getDeviceLocation();
 ...
 const toggleCamera = async () => {
   setCameraOrientation(prev => prev === "user" ? "environment" : "user");
   if (scanning) {
 ...
 const captureImage = async () => {
   ...
   try {
     ...
     try {
       // If in a WebView with reversed dimensions issue, adjust drawing
       const platform = getPlatformInfo();
       if (platform.isAndroid && platform.isWebView) {
         // Some Android WebViews need special handling for orientation
         context.save();
         if (cameraOrientation === "environment") {
           // Additional transformations for rear camera if needed
         }
         context.drawImage(video, 0, 0, canvas.width, canvas.height);
         context.restore();
       } else {
         // Standard drawing for browsers
         context.drawImage(video, 0, 0, canvas.width, canvas.height);
       }
     } catch (drawError) {
 ...
 return <div className="max-w-3xl mx-auto pb-20 md:pb-6 px-3 sm:px-4 md:px-6">
     ...
     duration: 0.3
                 ...
                 {cameraError && <div className="absolute inset-0 bg-black bg-opacity-70 flex flex-col items-center justify-center text-white p-4">
                     ...
                     <div className="flex gap-2">
                       ...
                       <button onClick={() => {
                         ...
                         setCameraOrientation(prev => prev === "user" ? "environment" : "user");
                         setTimeout(startCamera, 1000);
                       }} className="px-4 py-2 bg-green-600 rounded-lg text-sm"><span className="editable-text">
                         Ganti Kamera
                       </span></button>
                     </div>
                   </div>}
         ...
         <div className="p-6 grid grid-cols-1 md:grid-cols-2 gap-4">
           ...
           {capturedImage && photoTaken && recognizedTeacher && !processingCapture && <button onClick={submitAttendance} className="py-3 bg-green-600 text-white rounded-lg font-medium hover:bg-green-700 transition-colors flex items-center justify-center gap-2" disabled={processingCapture}>
               <CheckCircle size={20} />
         ...
         <div className="ml-3">
           ...
           <div className="mt-2 text-sm text-yellow-700">
             <ul className="list-disc pl-5 space-y-1">
               ...
               <li><span className="editable-text">Jika kamera tidak tampil, klik tombol "Ganti Kamera"</span></li>
             </ul>
           </div>
         </div>
       </div>
