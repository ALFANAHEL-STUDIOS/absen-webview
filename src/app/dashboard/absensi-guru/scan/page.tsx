...
import { Camera, MapPin, User, AlertCircle, ArrowLeft, Loader2, CheckCircle, Timer, LogIn, LogOut, X, RefreshCw, MapPinOff } from "lucide-react";
...
export default function TeacherAttendanceScan() {
 ...
 useEffect(() => {
   ...
   loadSettings();
   setLoading(false);
   // Pre-request permissions for WebView
   if (isAndroidWebView()) {
 ...
 const isAndroidWebView = () => {
   ...
   const isWebView = /wv|webview|crosswalk|tizen/i.test(userAgent) ||
                     (typeof window !== 'undefined' &&
                     typeof window.ReactNativeWebView !== 'undefined');

   return isAndroid && isWebView;
 };
 // Early permission request for WebView
 const requestLocationPermissionEarly = () => {
   try {
     ...
     if (navigator.geolocation) {
       ...
       toast("Izin lokasi diperlukan, silakan klik 'Izinkan' saat diminta", {
         duration: 5000,
         icon: "📍"
       });
         ...
         (err) => {
           ...
           navigator.geolocation.getCurrentPosition(
             ...
             (err2) => {
               ...
               setLocationError(`Gagal mendapatkan izin lokasi: ${err2.message}`);
               setIsRequestingLocation(false);
             },
     ...
     console.error("Error pre-requesting location permission:", error);
   }
 };
 ...
 const getPlatformInfo = () => {
   ...
   const isWebView = () => {
     ...
     const isIOSWebView = (/iphone|ipod|ipad/.test(userAgent) && !window.navigator.standalone) ||
                          (typeof window.navigator.standalone === 'boolean' && window.navigator.standalone === false);
     // Check for special window variables that might indicate a webview
     const hasWebViewBridge =
       typeof window.ReactNativeWebView !== 'undefined' ||
       typeof window.webkit?.messageHandlers !== 'undefined' ||
       /wv|webview|crosswalk|tizen/i.test(userAgent);
 ...
 const startCamera = async () => {
   try {
     ...
     // Get location with enhanced access
     getDeviceLocation();
   } catch (error: any) {
 ...
 // Enhanced location access for Android WebView
 const getDeviceLocation = () => {
   ...
   // Set a flag to show we're requesting location
   setIsRequestingLocation(true);
   setLocationError(null);

   // First try high accuracy settings with shorter timeout
   ...
   // For WebView performance, add a loading message
   if (isAndroidWebView()) {
     setLocationMessage("Meminta akses lokasi...");
   }
   ...
   navigator.geolocation.getCurrentPosition(
     ...
     // Error callback - try again with lower accuracy on failure
     error => {
       console.log("High accuracy location request failed:", error.code, error.message);
       ...
       console.log("WebView detection:", isAndroidWebView());
       console.log("User agent:", navigator.userAgent);

       navigator.geolocation.getCurrentPosition(
         // Success with lower accuracy
         position => {
           const userLocation = {
             lat: position.coords.latitude,
             lng: position.coords.longitude
         ...
         // Final error - both attempts failed
         finalError => {
           console.error("Geolocation error:", finalError);
           setIsRequestingLocation(false);
           setLocationPermission(false);
           ...
           switch (finalError.code) {
             case 1: // PERMISSION_DENIED
               errorMsg += "Izin lokasi ditolak. Harap aktifkan izin lokasi di pengaturan perangkat atau browser Anda.";
               break;
             case 2: // POSITION_UNAVAILABLE
               errorMsg += "Informasi lokasi tidak tersedia. Coba aktifkan GPS atau coba di luar ruangan.";
               break;
             case 3: // TIMEOUT
               errorMsg += "Waktu permintaan lokasi habis. Periksa koneksi internet dan GPS Anda.";
               break;
             default:
               errorMsg += "Pastikan GPS diaktifkan dan izin lokasi diberikan.";
           }
   ...
   // Set a backup timeout for WebView environments
   // This helps in case the geolocation callbacks don't fire properly in WebView
   if (isAndroidWebView()) {
     locationTimeoutRef.current = setTimeout(() => {
       if (isRequestingLocation) {
         setIsRequestingLocation(false);
         setLocationError("Waktu permintaan lokasi habis. Coba buka pengaturan lokasi perangkat dan aktifkan akses 'Presisi Tinggi'");
         toast.error("Gagal mendapatkan lokasi. Coba refresh halaman ini atau periksa izin lokasi di pengaturan.", {
           duration: 5000
         });
       }
 ...
 const retryGetLocation = () => {
   setLocationError(null);
   setLocationMessage("Mencoba mendapatkan lokasi kembali...");
   getDeviceLocation();
 };
 // Stop camera
 const stopCamera = () => {
 ...
 return (
   <div className="max-w-3xl mx-auto pb-20 md:pb-6 px-3 sm:px-4 md:px-6">
     ...
     {loading ? (
       ...
       >
         ...
         <p className="text-gray-600 mb-6">
           ...
           {attendanceType === 'in' ? 'masuk' : 'pulang'}<span className="editable-text">.
         </span></p>
         <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
           ...
           <Link
             href="/dashboard/absensi-guru/attendance-table"
       ...
       <div className="bg-white rounded-xl shadow-md overflow-hidden">
             ...
             {scanning ? (
               ...
               <div className="flex flex-col items-center justify-center h-full">
                 ...
                 <p className="text-gray-400">
                   <span className="editable-text">Kamera belum diaktifkan</span>
                 </p>
               </div>
             )}
           ...
           <div className={`p-4 mb-4 rounded-lg flex items-center justify-between ${
             ...
             <div className="flex items-center">
               {isRequestingLocation ? (
                 ...
                 <MapPinOff className="h-5 w-5 mr-2" />
               )}
           ...
           {recognizedTeacher && (
             <div className="p-4 bg-blue-50 rounded-lg mb-4 border border-blue-200">
               ...
               <p className="text-sm text-blue-600">
                 <span className="editable-text">NIK: </span>{recognizedTeacher.nik}
               </p>
               <p className="text-sm text-blue-600">
                 <span className="editable-text">Jabatan: </span>{recognizedTeacher.role}
               </p>
             </div>
           )}
         </div>
         ...
         <div className="p-6 grid grid-cols-1 md:grid-cols-2 gap-4">
           {!scanning && !capturedImage && (
             <button
               onClick={startCamera}
               className="col-span-full py-3 bg-primary text-white rounded-lg font-medium hover:bg-primary hover:bg-opacity-90 transition-colors flex items-center justify-center gap-2"
             >
               <Camera size={20} />
           ...
           {capturedImage && photoTaken && recognizedTeacher && !processingCapture && (
             <button
               ...
               disabled={processingCapture || !location}
             >
               <CheckCircle size={20} />
           ...
           {processingCapture && (
             <div className="col-span-full flex items-center justify-center py-3 bg-gray-300 text-gray-700 rounded-lg font-medium">
         ...
         <div className="ml-3">
           ...
           <div className="mt-2 text-sm text-yellow-700">
             <ul className="list-disc pl-5 space-y-1">
               ...
               <li><span className="editable-text">Aktifkan GPS pada perangkat Anda dengan pengaturan akurasi tinggi</span></li>
               <li><span className="editable-text">Izinkan akses lokasi ketika diminta oleh browser</span></li>
             </ul>
           </div>
         </div>
       </div>
